...
What it does:
  1) Builds signed Maven artifacts into build/central-bundle-repo/
  2) Generates .md5 and .sha1 for repository files (excluding *.asc/*.md5/*.sha1)
  3) Creates build/central-bundle.zip
  4) Uploads the bundle to Sonatype Central Portal (publishingType=AUTOMATIC)
  5) Waits for Central Portal to finish publishing (unless --no-wait)
...
main() {
  ...
  ./gradlew :ratchet:publishMavenJavaPublicationToCentralBundleRepository :ratchet-android:publishMavenJavaPublicationToCentralBundleRepository --no-daemon --stacktrace
